...
class ArticleServiceTest {
    ...
    void index() {
        ...
        List<Article> expected=new ArrayList<>(Arrays.asList(a,b,c));       //a,b,c 합치기. asList(): 입력된 배열 또는 2개 이상의 동일한 타입 데이터를 정적 리스트로 만들어 반환함.
    ...
    void show_실패_존재하지_않는_id_입력(){
        //1. 예상 데이터
        Long id=-1L;   //id가 -1인 것은 없으므로
        Article expected=null;     //예상 데이터 null 저장. DB에 조회되는 내용이 없어 null 반환할 것이기 때문에.
        //2. 실제 데이터
        Article article=articleService.show(id);
        //3. 비교 및 검증
        assertEquals(expected,article);     //null은 toString()메서드를 호출할 수 없으므로 그냥 expected와 article사용.
