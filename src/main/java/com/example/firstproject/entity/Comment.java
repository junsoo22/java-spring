...
public class Comment{
    ...
    @JoinColumn(name="article_id")     //외래키 생성, ARticle 엔티티의 기본키(id)와 매핑. Primary key와 알아서 매핑해줌
    ...
    public static Comment createComment(CommentDto dto, Article article) {
        ...
        if(dto.getArticleId() != article.getId())     //dto에서 가져온 부모게시글ㅇ과 엔티티에서 가져온 부모 게시글의 id가 다르면 안됨.
    ...
    public void patch(CommentDto dto) {
        //예외 발생
        if(this.id!=dto.getId())        //URL에 있는 id와 JSON 데이터의 id가 다른 경우
